...
public class UserService {
    ...
    public UserDTO findById(Long id) {
        // Busca um usuário pelo ID no repositório e converte para UserDTO.
        // O método findById retorna um Optional<User>, que é uma forma segura de lidar com a possibilidade de o usuário não existir.
        Optional<User> usuario = userRepositorio.findById(id);
        if (usuario.isPresent()) {
            return UserDTO.convert(usuario.get());
        }
        // Se o usuário for encontrado, converte a entidade User para UserDTO e retorna.
